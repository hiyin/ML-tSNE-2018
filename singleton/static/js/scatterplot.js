(function(exports)
{
	...
	*/
	...
	var ScatterPlot = function(params) {
		...
		this.circleRadius = params['circleRadius']==null? 3.5 : params['circleRadius'];
	}
	...
	ScatterPlot.prototype.draw = function(data)
	{
		...
		self.xScale = d3.scale.linear()
			.domain(d3.extent(self.data.map(function(d) { return d.x; })))
			.range([margin.left, width-margin.right]);

		self.yScale = d3.scale.linear()
			.domain(d3.extent(self.data.map(function(d) { return d.y; })))
		...
		self.zoom = d3.behavior.zoom()
			...
			self.svg.selectAll(".dot").attr("transform", function(d) { return "translate(" + self.xScale(d.x) + "," + self.yScale(d.y) + ")"; });
    ...
    ScatterPlot.prototype.renderPoints = function()
    {
		var self = this;

        // Create circle points
        self.circle = self.svg.selectAll(".dot")
			.data(self.data)
		    .enter().append("circle")
			.attr("class", "dot")
	...
	// Create a colour coded legend for the different samples
	ScatterPlot.prototype.createLegend = function(colour)
	{
		...
		legend.append("rect")
		.attr("x", this.svg.attr('width') - 120)
		.attr("width", 18)
		.attr("height", 18)
		.style("fill", colour);

		legend.append("text")
		.attr("x", this.svg.attr('width') - 126)
		.attr("y", 9)
		.attr("dy", ".35em")
